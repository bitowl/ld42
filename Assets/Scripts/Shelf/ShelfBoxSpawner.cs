...
public class ShelfBoxSpawner : MonoBehaviour {
	...
	private void spawnBox(int x, int y) {
		GameObject.Instantiate(BoxPrefab, transform.position + transform.rotation * new Vector3(x + SingleBoxOffsetX, y, 0), transform.rotation);
	}

	private void spawnDoubleBox(int x, int y) {
		GameObject.Instantiate(DoubleBoxPrefab, transform.position + transform.rotation * new Vector3(2 * x + DoubleBoxOffsetX, y, 0), transform.rotation);
	}

	private void spawnQuadBox(int x, int y) {
		GameObject.Instantiate(QuadBoxPrefab, transform.position + transform.rotation * new Vector3(4 * x + QuadBoxOffsetX, y, 0), transform.rotation);
	}
